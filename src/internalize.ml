...
let bind (env : env) (id : identifier) : atom * env =
...
let mktc tctable env id tys =
  ...
  if expected <> got then
    Error.signal
      ...
      (Printf.sprintf
         "The type constructor %s expects %d arguments, but is here applied to \
          %d arguments.\n"
...
let ityvar env a = TyFreeVar (Import.resolve env a)
let _ityvars env = List.map (ityvar env)
...
and itypes tctable env tys = List.map (itype tctable env) tys
...
let rec iterm tctable env = function
    ...
    | SynTeJump (x, tyargs, fields, ty) ->
        TeJump
          ( Import.resolve env x,
            List.map (Import.resolve env) tyargs,
            iterms tctable env fields,
            itype tctable env ty )
    | SynTeJoin (x, tyargs, binds, ty, terms1, terms2) ->
        let x, env' = bind env x in
        let binds_atom, binds_type = List.split binds in
        let env'', tyargs =
          List.fold_left_map
            (* to apply fold left we have to reverse the output of bind *)
            (fun env ident ->
              let a, env' = bind env ident in
              (env', a))
            env tyargs
        in
        let env''', binds_a =
        ...
        TeJoin
          ( x,
            tyargs,
            binds_a,
            itypes tctable env'' binds_type,
            itype tctable env'' ty,
            iterm tctable env''' terms1,
            iterm tctable env' terms2 )
...
let build ds : type_table * datacon_table * env =
  ...
  let tcs : identifier list =
    List.fold_left
      (fun tcs -> function SynType (tc, _) -> tc :: tcs | _ -> tcs)
      [] ds
  in
  ...
  let tctable : int AtomMap.t =
    List.fold_left
      (fun tctable -> function
        | SynType (tc, params) ->
            AtomMap.add (Import.resolve env tc) (List.length params) tctable
        | _ -> tctable)
      AtomMap.empty ds
  in
  ...
  let dcs : identifier list =
    List.fold_left
      (fun dcs -> function SynDatacon (d, _) -> d :: dcs | _ -> dcs)
      [] ds
  in
  ...
  let dctable : ftype AtomMap.t =
    List.fold_left
      (fun dctable -> function
        | SynDatacon (d, s) ->
            AtomMap.add (Import.resolve env d) (ischeme tctable env s) dctable
        | _ -> dctable)
      AtomMap.empty ds
  in
