...
module AM = Atom.AtomMap
module IM = Identifier.Map
...
type env = { mapping : identifier AM.t; salt : int IM.t }

(* The empty environment. *)

let empty = { mapping = AM.empty; salt = IM.empty }

(* [bind env a] extends the environment [env] with a mapping of
   the atom [a] to a unique identifier. *)

let bind { mapping; salt } a =
  let basename : identifier = Identifier.basename (Atom.identifier a) in
  let grain : int = try IM.find basename salt with Not_found -> 0 in
  let id : identifier = Identifier.combine basename grain in
  { mapping = AM.add a id mapping; salt = IM.add basename (grain + 1) salt }
