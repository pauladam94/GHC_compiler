...
let pvar env x = string (Identifier.name (Export.resolve env x))
...
let arrow = string " ->"
let lambda = string "fun"
let colon = string " :" ^^ softline
let equal = string " ="
let kmatch = string "match "
let return = softline ^^ string "return" ^^ softline
let kwith = string " with" ^^ line
let kend = string "end"
let bar = string "| "
...
let heading head body = group (nest 2 (group head ^^ linebreak ^^ body))
...
let jump body = group (nest 2 (line ^^ body) ^^ line)
...
let pfields f fields =
  ...
  | _ ->
      braces
        (jump
           (separate_map (semi ^^ softline)
              (* [softline] allows several fields per line, with wrapping *)
...
let app f2 t1 t2 = t1 ^^ line ^^ f2 t2
let apps f2 t1 t2s = List.fold_left (app f2) t1 t2s
...
let rec pty0 env ty =
  ...
  | TyCon (tc, []) -> pvar env tc
  | TyTuple fields -> pfields (pty env) fields
  | _ -> parens (pty env ty)

and pty1 env ty =
  match ty with
  | TyCon (tc, tys) -> apps (pty0 env) (pvar env tc) tys
  | _ -> pty0 env ty
...
and pty env ty =
  group
    ...
    | TyArrow (domain, codomain) ->
        pty1 env domain ^^ arrow ^^ softline ^^ pty env codomain
...
and brackets_pty env ty = brackets (pty env ty)

let brackets_pvar env a = brackets (pvar env a)
...
let rec pterm0 env = function
    | TeLoc (_, term) -> pterm0 env term
    | TeVar (x, _) -> pvar env x
    | TeData (dc, tys, fields, _) ->
        app
          (pfields (pterm env))
          (apps (brackets_pty env) (pvar env dc) tys)
          fields
    | TeTyAnnot (term, ty) -> parens (pterm env term ^^ colon ^^ pty env ty)
...
and pterm1 env term =
  group
    (match term with
    | TeLoc (_, term) -> pterm1 env term
    | TeApp (term1, term2, _) -> pterm1 env term1 ^^ line ^^ pterm0 env term2
    | TeTyApp (term, ty, _) -> pterm1 env term ^^ line ^^ brackets_pty env ty
    | _ -> pterm0 env term)

and pterm env term =
  group
    (match term with
    | TeLoc (_, term) -> pterm env term
    | TeAbs (x, domain, body) -> begin
        try
...
and pterm_with_dc env dc term =
  ...
  | DC (ty_args, [], oty), TeTyAbs (a, body) ->
      ...
      pterm_with_dc env (DC (ty_args @ [ a ], [], instantiate_oty oty a)) body
    ...
    | Clause (PatData (_, dc, tyvars, tevars, _), term) ->
        ...
        nest 4
          (group
             ...
             ^^ app
                  (pfields (pvar env))
                  (apps (brackets_pvar env) (pvar env dc) tyvars)
                  tevars
             ^^ arrow)
          ^^ line ^^ pterm env term)
        ^^ line
...
let print_tc p env tc arity =
  ...
  AtomSet.fold
    (fun dc accu -> print_dc p env dc ^^ linebreak ^^ accu)
...
let print_program (Prog (tctable, dctable, term) as p) =
  ...
  AtomMap.fold
    (fun tc arity accu -> print_tc p env tc arity ^^ accu)
    tctable empty
