...
and ftype_context = Identifier.identifier * ftype
...
let rec equal ty1 ty2 =
  match (ty1, ty2) with
  | TyBoundVar i1, TyBoundVar i2 -> i1 = i2
  ...
  | TyForall (_, body1), TyForall (_, body2) -> equal body1 body2
  | TyCon (tc1, tys1), TyCon (tc2, tys2) -> begin
      try
        ...
        id1 = id2 && List.for_all2 equal tys1 tys2
      with Invalid_argument _ ->
        assert false (* arity error: should not happen *)
    end
  | TyTuple tys1, TyTuple tys2 -> begin
      try List.for_all2 equal tys1 tys2
      with Invalid_argument _ ->
...
let rec abstract a i ty =
  ...
  | TyForall (hint, body) -> TyForall (hint, abstract a (i + 1) body)
  | TyCon (tc, tys) -> TyCon (tc, List.map (abstract a i) tys)
  | TyTuple tys -> TyTuple (List.map (abstract a i) tys)
...
let rec fill i ty c =
  ...
  | TyBoundVar _ | TyFreeVar _ -> c
  | TyArrow (domain, codomain) -> TyArrow (fill i ty domain, fill i ty codomain)
  | TyForall (hint, body) -> TyForall (hint, fill (i + 1) ty body)
  | TyCon (tc, tys) -> TyCon (tc, List.map (fill i ty) tys)
  | TyTuple tys -> TyTuple (List.map (fill i ty) tys)

let fill (_, c) ty = fill 0 ty c
...
let empty : tenv = AtomMap.empty

let lookup : atom -> tenv -> ftype =
 fun x tenv ->
  try AtomMap.find x tenv
  with Not_found ->
    assert false (* should not happen, if there is a binding of [x] in [tenv] *)

let bind : atom -> ftype -> tenv -> tenv = AtomMap.add
...
type jenv = (atom list * ftype list) AtomMap.t
...
let rec count_foralls accu = function
    | TyForall (_, body) -> count_foralls (accu + 1) body
    | _ -> accu
